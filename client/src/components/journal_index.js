...
class JournalIndex extends Component {
  ...
  renderJournals(){
        return this.props.journals.map( journal => <Link to={'/'+ journal.id} className='journals'key={shortid.generate()}>{journal.title}</Link> )
  }
           ...
           <div className="row">
               <div className="col-sm-2 offset-sm-5">
                   <button
                     ...
                     className="btn btn-outline-primary ">New Journal</button>
               </div>
           </div>
            {this.renderJournals()}
        </div>
